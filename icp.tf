...
resource "null_resource" "icp-cluster" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo /tmp/icp-common-scripts/download_docker.sh ${var.icp_source_server} ${var.icp_source_user} ${var.icp_source_password} ${var.image_file} ${var.install_dir}/images/${basename(var.docker_installer)}",
